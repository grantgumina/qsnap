...
use snapshot_description::SnapshotDescription as SnapshotDescription;
use qsnaperror::QsnapError as QsnapError;

fn list_snapshots(matches: &ArgMatches) -> Result<(), QsnapError> {

        if_chain! {
            
            if let ip_addresses = matches.values_of("node_ip_addresses").unwrap();
            if let region = matches.value_of("region").unwrap();
            ...
            then {
                ...
                match snapshot_descriptions.snapshots {
                    Some(snapshots) => {
                        ...
                        let mut qumulo_snapshot_descriptions: Vec<SnapshotDescription> = vec![];
                    ...
                    None => {
                        Err(QsnapError::new(constants::EC2_SNAPSHOTS_NOT_FOUND_ERROR))
                    }
                }

            } else {
                Err(QsnapError::new("foo"))
            }
...
fn main() {
    let matches = App::new("qnsap")
                    ...
                    .subcommand(SubCommand::with_name("snapshot")
                        ...
                        .arg(Arg::with_name("region")
                            ...
                            .help("AWS region code. See https://docs.aws.amazon.com/general/latest/gr/rande.html for more details.")
                        )
                    )
    ...
    let result: Result<(), QsnapError> = match matches.subcommand() {
        // ("snapshot", Some(m)) => snapshot_cluster(m),
        // ("restore", Some(m)) => restore_cluster(m),
        ("list", Some(m)) => list_snapshots(m),
        _ => Err(QsnapError::new(constants::QSNAP_DEFAULT_HELP_MESSAGE))
    };
